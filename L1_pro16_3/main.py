...
def main():
    ...
    #model = resnet.__dict__[args.model]().cuda()
    ...
    for name,param in model.named_parameters():
        if 'conv' in name and 'layer' in name: # all conv layers except the first one
           conv_list.append(param.shape[0]) 
    for it in range(len(conv_list)):
        alpha_list.append(torch.nn.Parameter(torch.ones(conv_list[it])*0.1,requires_grad=True))
    model.L1_alpha = torch.nn.ParameterList(alpha_list)
    ...
    optimizer = torch.optim.SGD([{'params':model.parameters(),'initial_lr':args.lr}], args.lr,
                                momentum=0.9,
                                weight_decay=1e-4) #* apply L1-norm
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        L1_alpha_mean = 0
        for i_mean in model.L1_alpha:
            L1_alpha_mean+=i_mean.mean()
        L1_alpha_mean/=len(model.L1_alpha)
        writer.add_scalar('L1_alpha',L1_alpha_mean,epoch)
        
        #* Tracking weights
        if epoch%10==0:
        # png_path=os.path.join(save_path, 'images')
...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None,beta_distribution=None):
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        for i_alpha,param in enumerate(conv_parameters):
            for i_each,each_param in enumerate(param):
                L1_norm += torch.sum(torch.abs(torch.abs(each_param)-model.L1_alpha[i_alpha][i_each]))
        loss = loss + T * L1_norm
