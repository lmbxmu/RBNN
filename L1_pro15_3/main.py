...
def main():
    ...
    N_conv=0 #* compute the number of conv layers
    for name,param in model.named_modules():
        if 'conv' in name and 'layer' in name: # all conv layers except the first one
            N_conv+=1
    model.L1_alpha = torch.nn.Parameter(torch.ones(N_conv)*0.1, requires_grad=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        writer.add_scalar('L1_alpha',model.L1_alpha.mean(),epoch)
...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None,beta_distribution=None):
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        for i_alpha,param in enumerate(conv_parameters):
            L1_norm += torch.sum(torch.abs(torch.abs(param)-model.L1_alpha[i_alpha]))
        loss = loss + T * L1_norm
